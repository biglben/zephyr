			       ...
			       uint8_t node_cnt, uint8_t burst_cnt, uint8_t burst_timer)
{
	...
	regVal = ((uint32_t)node_cnt << 8) | node_id;
	oa_tc6_reg_write(ctx->tc6, 0x0004CA02, regVal); /* PLCA_CONTROL_1_REGISTER */

	/* PLCA Burst Count and Burst Timer */
	regVal = ((uint32_t)burst_cnt << 8) | burst_timer;
	oa_tc6_reg_write(ctx->tc6, 0x0004CA05, regVal); /* PLCA_BURST_MODE_REGISTER */
